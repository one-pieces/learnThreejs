import { clamp, color, Fn, If, mix, normalWorld, positionLocal, positionWorld, smoothstep, step, uniform, uv, vec2, vec3, vec4 } from "three/tsl";
...
export const toTriplanarUv = Fn(([position, mask]) => {
...
export const toAntialiasedGrid = Fn(([uv, scale, thickness, offset, cross, derivateMask]) => {
    ...
    const corssGrid = step(referenceUv.fract().sub(0.5).abs(), cross.oneMinus().mul(0.5))
    const crossMask = mix(corssGrid.x, 1, corssGrid.y).oneMinus()
...
export class MeshGridMaterial extends NodeMaterial {
